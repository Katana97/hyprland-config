...
### 6.3 eGPU Hot-Plug Workflow (The Solution!)

**The NVIDIA driver loads during early boot before the eGPU is fully enumerated over Thunderbolt.** This means the eGPU won't be detected on boot even though it's connected.

**The Simple Solution:** Hot-plug after boot!

You have two options:

**Option A: eGPU Connected at Boot**
1. Boot normally with eGPU connected
2. Wait ~10 seconds for desktop to load
3. Unplug the Thunderbolt cable
4. Wait 2-3 seconds
5. Plug it back in
6. ✅ All 3 GPUs detected!

**Option B: eGPU Unplugged at Boot (Recommended)**
1. Boot with eGPU unplugged (faster, saves power)
2. When ready to game, plug in Thunderbolt cable
3. ✅ All 3 GPUs detected!

**Verify it worked:**
```bash
switcherooctl list    # Should show all 3 GPUs
nvidia-smi           # Should show both NVIDIA GPUs
```

This hot-plug approach is **reliable, simple, and works 100% of the time**. No complex systemd services or scripts needed!
...
### 9.1 eGPU Not Detected

**If eGPU isn't showing up:**
...
# Check PCI bus
lspci | grep -E "VGA|3D"
# Should show RTX 4070 Ti at 08:00.0
...
**Solution:** Unplug and replug the Thunderbolt cable. Wait 5 seconds, then check again.

This triggers a PCIe hot-plug event that forces NVIDIA driver re-initialization.
...
## Part 13: Create System Backup/Restore Point

Before installing any dotfiles, create a complete backup so you can easily revert.

### 13.1 Using Timeshift (Recommended)

Timeshift is the easiest way to create system snapshots on CachyOS:
...
In Timeshift:
1. Select **BTRFS** (if your system uses BTRFS) or **RSYNC**
2. Choose your backup location
3. Click **Create** snapshot
4. Add comment: "Before end-4 dotfiles - GPU working perfectly"

**To restore later:**
1. Boot from CachyOS live USB
2. Run `sudo timeshift-gtk`
3. Select your snapshot
4. Click **Restore**

### 13.2 Manual Backup (Works on Any Filesystem)

```bash
# Backup Hyprland config
cp -r ~/dotfiles ~/dotfiles.backup-$(date +%Y%m%d)
# OR if config is in ~/.config:
...
# Save package list
pacman -Qe > ~/backups/package-list-$(date +%Y%m%d).txt

# Save GPU configuration for reference
cp /etc/udev/rules.d/99-* ~/backups/ 2>/dev/null || true
cat ~/dotfiles/hypr/.config/hypr/hyprland.conf > ~/backups/hyprland-working.conf
```

### 13.3 Test Your Backup

Before proceeding, verify your backup is complete:

```bash
ls -lh ~/backups/
# Should show your tar.gz backup and config files

# Test that tar file is valid
tar -tzf ~/backups/config-backup-$(date +%Y%m%d).tar.gz | head
# Should list files inside the archive
```

### 13.4 Document Current Working State

Create a quick reference of what's working:

```bash
cat > ~/backups/WORKING_STATE.txt << 'EOF'
WORKING CONFIGURATION - $(date)
================================

GPU Setup:
- RTX 4070 Ti: Working via hot-plug (unplug/replug after boot)
- GTX 1650 Ti: Working
- Intel UHD: Working as compositor

Verified Working:
- vkcube uses RTX 4070 Ti (GPU 1 in nvidia-smi)
- Journey (Steam) uses RTX 4070 Ti
- XWayland apps display clearly (no blur)
- Fractional scaling: 1.25x

Critical Config Files:
- ~/dotfiles/hypr/.config/hypr/hyprland.conf
  - Contains: MESA_VK_DEVICE_SELECT=10de:2782
  - Contains: XWayland scaling fixes

Hot-Plug Ritual:
1. Boot (eGPU can be plugged or unplugged)
2. After login, unplug Thunderbolt cable
3. Wait 2-3 seconds
4. Plug back in
5. Verify: switcherooctl list (should show 3 GPUs)
EOF
```
